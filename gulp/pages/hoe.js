...
module.exports = function hoeTask(projectRoot) {
  ...
  const hoeRoot = path.resolve(projectRoot, "./hoe/");
  const hoeDest = path.resolve(projectRoot, "./hoe/build/");
  const serverDir = path.resolve(hoeRoot, "./server/");
  const simulatorServerDir = path.resolve(projectRoot, "./simulator/server/");
  const controlsServerDir = path.resolve(projectRoot, "./controls/server/");
  ...
  gulp.task("hoe-build", function(){
    ...
    const cssFiles = [
      ...
      `${projectRoot}/simulator/client/scss/**/*.scss`,
      `${projectRoot}/simulator/client/js/**/*.scss`,
      `${projectRoot}/controls/client/scss/**/*.scss`,
      `${projectRoot}/controls/client/js/**/*.scss`
    ];
